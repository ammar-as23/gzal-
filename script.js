...
async function saveOrderToDatabase(orderData) {
    const newOrder = {
        ...
        status: 'pending' // قيد المراجعة افتراضياً
    };
    ...
    try {
        ...
        console.error('❌ حدث خطأ أثناء إرسال الطلب للسيرفر:', error);
        return false;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    loadProducts(); // جلب المنتجات عند فتح الصفحة
    renderCart();
    
    const orderForm = document.getElementById('orderFormElement');
    if (orderForm) {
        // تحويل الحدث إلى async لاستخدام await
        orderForm.addEventListener('submit', async function(e) {
            ...
            const itemsText = cart.map(item => `${item.name}${item.hasVip ? ' (+VIP)' : ''} × ${item.quantity}`).join(' | ');
            ...
            if (isSuccess) {
                ...
                alert('❌ حدث خطأ أثناء الإرسال، يرجى التحقق من اتصالك بالإنترنت والمحاولة مجدداً.');
            }
        });
    }
});// ================================================
